...
def build_graph(
    ...
    # MultiDiGraph preserves parallel edges (the same pair of entities can be
    # related by distinct relationship types, e.g. used_with AND prevents).
...
def _graph_stats(g: nx.MultiDiGraph) -> dict[str, Any]:
...
def run_build_graph() -> None:
    ...
    graph_json = {
        ...
        "edges": [
            {"from_id": a, "to_id": b, "relationship": data.get("relationship", "")}
            for a, b, _k, data in g.edges(data=True, keys=True)
        ],
    }
